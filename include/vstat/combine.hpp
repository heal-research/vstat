...
namespace detail {
    template<typename T>
    struct is_vcl_type {
        static constexpr bool value = detail::is_any_v<T, Vec2d, Vec4f, Vec4d, Vec8f, Vec8d, Vec16f>;
    };
...
template<typename T, std::enable_if_t<detail::is_vcl_type_v<T>, bool> = true>
inline auto combine(T sum_w, T sum_x, T sum_xx) -> double
{
    if constexpr (T::size() == 2) {
        ...
        return horizontal_add(sum_xx) + f * detail::square(n1 * s0 - n0 * s1); // eq. 22
