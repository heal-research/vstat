...
inline univariate_statistics accumulate(InputIt1 first, InputIt1 last, F&& f = F{}) noexcept
{
    ...
    if (n < s) {
        univariate_accumulator<T> scalar_acc(f(*first++));
        while (first < last) scalar_acc(f(*first++));
    ...
    std::advance(first, s);
    univariate_accumulator<vec> acc(vec().load_a(x.data()));

    for (size_t i = s; i < m; i += s) {
        std::transform(first, first + s, x.begin(), f);
        acc(vec().load_a(x.data()));
        std::advance(first, s);
    }
    ...
    if (m < n) {
        ...
        for (auto it = first + m; it != last; ++it) {
            scalar_acc(f(*it));
        }
        return univariate_statistics(scalar_acc);
    }
...
inline univariate_statistics accumulate(InputIt1 first1, InputIt1 last1, InputIt2 first2, F&& f = F{}) noexcept
{
    ...
    if (n < s) {
        univariate_accumulator<T> scalar_acc(f(*first1++), *first2++);
        while (first1 < last1) scalar_acc(f(*first1++), *first2++);
        return univariate_statistics(scalar_acc);
    }
    ...
    univariate_accumulator<vec> acc(vec().load_a(x.data()), vec().load_a(w.data()));

    for (size_t i = s; i < m; i += s) {
        std::transform(first1, first1 + s, x.begin(), f);
        std::copy(first2, first2 + s, w.begin());
        acc(vec().load_a(x.data()), vec().load_a(w.data()));
        std::advance(first1, s);
        std::advance(first2, s);
    }
...
inline bivariate_statistics accumulate(InputIt1 first1, InputIt1 last1, InputIt2 first2, F1&& f1 = F1{}, F2&& f2 = F2{}) noexcept
{
    ...
    if (n < s) {
        bivariate_accumulator<T> scalar_acc(f1(*first1++),f2(*first2++));
        while (first1 < last1) scalar_acc(f1(*first1++), f2(*first2++));
    ...
    bivariate_accumulator<vec> acc(vec().load_a(x1.data()), vec().load_a(x2.data()));

    for (size_t i = s; i < m; i += s) {
        std::transform(first1, first1 + s, x1.begin(), f1);
        std::transform(first2, first2 + s, x2.begin(), f2);
        acc(vec().load_a(x1.data()), vec().load_a(x2.data()));
        std::advance(first1, s);
        std::advance(first2, s);
    }
    ...
    if (m < n) {
        ...
        for (; first1 < last1; ++first1, ++first2) {
            scalar_acc(f1(*first1), f2(*first2));
        }
        return bivariate_statistics(scalar_acc);
    }
...
inline bivariate_statistics accumulate(InputIt1 first1, InputIt1 last1, InputIt2 first2, InputIt3 first3, F1&& f1 = F1{}, F2&& f2 = F2{}) noexcept
{
    ...
    if (n < s) {
        bivariate_accumulator<T> scalar_acc(f1(*first1++),f2(*first2++), *first3++);
        while (first1 < last1) scalar_acc(f1(*first1++), f2(*first2++), *first3++);
        return bivariate_statistics(scalar_acc);
    }
    ...
    bivariate_accumulator<vec> acc(vec().load_a(x1.data()), vec().load_a(x2.data()), vec().load_a(w.data()));

    for (size_t i = s; i < m; i += s) {
        std::transform(first1, first1 + s, x1, f1);
        std::transform(first2, first2 + s, x2, f2);
        std::copy(first3, first3 + s, w);
        acc(vec().load_a(x1.data()), vec().load_a(x2.data()), vec().load_a(w.data()));
        std::advance(first1, s);
        std::advance(first2, s);
        std::advance(first3, s);
    }
    ...
    if (m < n) {
        ...
        for (; first1 < last1; ++first1, ++first2, ++first3) {
            scalar_acc(f1(*first1), f2(*first2), *first3);
        }
        return bivariate_statistics(scalar_acc);
    }
    return bivariate_statistics(acc);
